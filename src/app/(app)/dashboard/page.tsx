    ...
    <div className="space-y-10 pb-10">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
         <div className='flex items-center gap-5'>
            ...
            <div>
                <h1 className="text-3xl font-extrabold tracking-tight text-foreground sm:text-4xl">Welcome, <span className="text-primary">{user?.firstName || 'Devotee'}</span></h1>
                <p className="text-muted-foreground text-lg mt-1">{greeting}</p>
            </div>
         </div>
       ...
       <div className="relative">
         {nextCountdown && (
            <div className="premium-card p-10 rounded-[32px] text-center relative overflow-hidden group">
                ...
                <h2 className="text-2xl sm:text-4xl font-black text-foreground tracking-tighter mb-8 flex items-center justify-center gap-3 sm:gap-4">
        ...
        {quickAccessLinks.map((item) => (
            <Link href={item.href} key={item.href} className="group">
                <div className="premium-card p-8 rounded-3xl h-full transition-all duration-300 hover:scale-[1.03] hover:border-primary/30 group-active:scale-[0.98]">
                    ...
                    <h3 className="font-black text-xl sm:text-2xl text-foreground mb-2">{item.title}</h3>
